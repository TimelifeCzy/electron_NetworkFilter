...
)
{
	...
	break;
	case ALPC_DRIVER_MSG_TEST:
	{
		printf("[entry: ALPC_DRIVER_MSG_TEST]\r\n");
		// 通过管道发给UI_DLL，由DLL将数据展示到UI界面
		DIRVER_MSG_TEST* InjectDllInject = (DIRVER_MSG_TEST*)((BYTE*)lpMem + sizeof(PORT_MESSAGE));
		wprintf(L"Driver recv buf: %s\r\n", InjectDllInject->MsgData);
		// g_pipui.PipSendMsg(InjectDllInject->MsgData, sizeof(MONITORCVEINFO));
	}
	break;
	...
	break;
	case ALPC_IPPACK_HEADER:
	{
		// 解析
		IPPACKHANDER* ippack = (IPPACKHANDER*)((BYTE*)lpMem + sizeof(PORT_MESSAGE));
		if (ippack)
		{
			// 998 代表非心跳探测, 正常的数据处理包
			ippack->heartbeat = 998;
			printf("[+]Tcp --> ProcessId : %d\t Protor: %d\t localAddressV4: 0x%x:%d\t remoteAddressV4: 0x%x:%d\r\n", ippack->pid, ippack->protocol, ippack->localaddr, ippack->localport, ippack->remoteaddr, ippack->remoteport);
			if (-1 == g_pipui.PipSendMsg(ippack, sizeof(IPPACKHANDER)))
			{
				printf("[3+]Client Pip Send Pip inactive!\r\n");
			}
		}
	}
	break;
...
)
{
	...
	if (!ntRet)
	{
		...
		if (!ntRet)
		{
			...
			{
				printf("Driver connect alpc success\r\n");
				// 发送上线成功消息/发送事件句柄
